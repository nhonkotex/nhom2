...
<%@page import="baoveCSDL.dao.NhanvienDao"%>

<%
   
    
	NhanvienDao dl = new NhanvienDao(DbCon.getConnetion());
    List<Nhanvien> nhanvien = dl.getAllNhanvien();
...
<section class="menu" id="menu">
    ...
    <h1 class="heading"> <span>All nhân viên Apple shop</span> </h1>
	...
	<%
			if(!nhanvien.isEmpty()) {
				...
				%>
        <div class="box">
            <img src="images/<%= q.getImage() %>" alt="">
            <h3><%= q.getNameNV() %></h3>
            <span>Chức Vụ: <%=q.getChucvu() %> </span> <br>
             <span>Phone: <%= q.getSdt() %></span>
             <p>Email: <%= q.getEmailNV() %> </p>
             <a class="btn btn-sm btn-danger" href="#CT">xem thêm</a>
			...
			if(!nhanvien.isEmpty()) {
				...
				%>
					<tr>
						...
						<td><%=q.getNameNV() %></td>
						<td><%=q.getEmailNV() %></td>
						<td><%=q.getPasswordNV() %></td>
						<td><%=q.getSdt() %></td>
						<td><%=q.getLuong() %> VNĐ</td>
						<td><%=q.getChucvu() %></td>
						<td><%=q.getDiachi() %></td>
